...
import { getId, getLecturerDetails } from "../../api/apiService";
  ...
  useEffect(() => {
    const loadLecturerData = async () => {
      try {
        ...
        if (id){
          const data = await getLecturerDetails(id)
          setLecturer(data);
        }
      } catch (error) {
      ...
      <View style={styles.container2}>
        <ImageBackground
          ...
          <View style={styles.sect1b}>
              <Text style={styles.sect1e}>{lecturer?.firstname} {lecturer?.lastname}</Text>
              <Text style={styles.sect1c}>ID: {lecturerId}</Text>
              <Text style={styles.sect1c}>{lecturer?.status}</Text>
          </View>
          <View style={styles.sect2}>
                <View style={styles.sect2a}>
                  <Text style={styles.text2b}>Faculty</Text>
                  <Text style={styles.text2a}>{lecturer?.faculty}</Text>
                </View>

                <View style={styles.sect2a}>
                  <Text style={styles.text2b}>Department</Text>
                  <Text style={styles.text2a}>{lecturer?.department}</Text>
                </View>

                <View style={styles.sect2a}>
                  <Text style={styles.text2b}>Status</Text>
                  <Text style={styles.text2a}>Lecturer</Text>
                </View>          
          </View>
